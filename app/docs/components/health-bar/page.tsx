  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col md:flex-row items-center justify-between gap-2">
        <h1 className="text-3xl font-bold">Health Bar</h1>
        <CopyCommandButton
          copyCommand={`pnpm dlx shadcn@canary add ${process.env.NEXT_PUBLIC_BASE_URL}/r/8bit-health-bar.json`}
          command={"pnpm dlx shadcn@canary add 8bit-health-bar"}
        />
      </div>

      <p className="text-muted-foreground">
        Displays an 8-bit health bar component.
      </p>

      <div className="flex flex-col gap-4 border rounded-lg p-4 min-h-[450px]">
        <div className="flex items-center justify-between">
          <h2 className="text-sm text-muted-foreground sm:pl-3">
            8-bit health bar component
          </h2>
      ...
      <CodeSnippet>{`import { HealthBar } from "@/components/ui/8bit/health-bar"`}</CodeSnippet>

      <CodeSnippet>{`<HealthBar value={75} />`}</CodeSnippet>
    </div>
  );
}
