...
export const mdxComponents = {
  ...
  img: ({ className, alt, ...props }: React.ComponentProps<"img">) => (
    // biome-ignore lint/correctness/useImageSize: flexible image size
    // biome-ignore lint/performance/noImgElement: img element is used for flexibility
  ...
  code: ({
    ...
    const getTextContent = (node: React.ReactNode): string => {
      ...
      if (React.isValidElement(node)) {
        const props2 = node.props as { children?: React.ReactNode };
        if (props2.children) {
          return getTextContent(props2.children);
        }
      }
      return "";
    };
